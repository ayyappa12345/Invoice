...
loggedUser;;
showloginpage=true;
updatedshowloginpage=new Subject<boolean>();
profile:MyProfile;
...
getupdatedshowloginSubs(){
  return this.updatedshowloginpage.asObservable()
}
...
registerUser(user:User){
  ...
  .subscribe(res=>{
    console.log(res);
    if(res){
      this.toasterServer.successMessage( "Registered Successful", "Success Messge");
...
getUpdatedUerLoggedSub(){
 ...
 constructor(private http:HttpClient, private router:Router, private toasterServer:ToasterService) { }
  getisAuth(){
...
logoutUser(){
 ...
 this.toasterServer.successMessage( "Logout Successful", "Success Messge");
...
clearTime(){
    ...
    this.http.post<{message:string, token:string, creator:string, expires:number}>(`${this.baseURL}login`, user).subscribe(res=>{
      if(res){
       ...
       if(token){
         ...
         this.router.navigate(['/products']);
         this.toasterServer.successMessage( "Login Successful", "Success Messge");
       }

        
      }
    }, err=>{
      this.toasterServer.errorMessage( err.error.message, "Error");
  ...
  loginpage(){
    ...
    this.updatedshowloginpage.next(true);
    console.log(this.showloginpage);
  }
  registerpage(){
    this.showloginpage=false;
    this.updatedshowloginpage.next(false);
    console.log(this.showloginpage);
  }
